...
try {
  ...
  $stmt= $conn->prepare("SELECT b.bill_id as ibill, i.price, item_name, quantity, i.ID, img, bi.user_id, bi.date FROM `bill` bi inner join `items` i inner join `bill_items` b on(b.`item_id`=i.`ID` AND b.`bill_id` = bi.`ID`) where bi.user_id=$uid");
