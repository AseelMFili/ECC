...
function addloc($lat,$lng){
	...
	if($uid!== false){
            $stmt = $conn->prepare("UPDATE `customers` SET `lat`= ? ,`lng`= ? WHERE ID = ? ");
...
function makeBill($email,$total){
     ...
     if($stmt->execute([$email]) && ($row=$stmt->fetch(PDO::FETCH_ASSOC)) !== false){
            ...
            $stmt = $conn->prepare("INSERT INTO bill (date, user_id, paid_flag, status,payment, total) VALUES (Now(), ?, 0, 'waiting for request','',?) ");
            
            $fieldsValues =[];
            $fieldsValues[]=$row['ID'];
            $fieldsValues[]=$total;
            if($stmt->execute([$row['ID'], $total])){
                ...
                /*$fieldsValues1 =[];
                $fieldsValues1[]=$id;
                $fieldsValues1[]=$row['ID'];*/
                
                if($stmt->execute([$id,$row['ID']])){
...
function paymentMethod($methodOfPay){
	...
	if($uid!== false){
	    $stmt = $conn->prepare("UPDATE bill SET `payment`=? where ID = $uid");
            
        if($stmt->execute([$methodOfPay])){
            echo "<script> window.location.href='https://ecc-test-aseelmfili.c9users.io/cartDB/bill_Page.php' </script>";
                }
            }
        }
        }catch(PDOException $e)
...
function changeBillStatus($status,$ID){
       ...
       $stmt = $conn->prepare("select * from bill where ID = ?");
        ...
        if($stmt->execute([$ID])){
           ...
           while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		    ...
		    if($status == "Pending"){
		        echo "<script> window.location.href='https://ecc-test-aseelmfili.c9users.io/Pending.php' </script>";
		    }else {
		        echo "<script> window.location.href='https://ecc-test-aseelmfili.c9users.io/Delivered.php' </script>";
		    }
     }
